...
public class AudioManager : SimulationManagerBase<AudioManager, AudioProperties>, IAudibleManager
{
		...
		while ((this.m_bufferReadPos - this.m_bufferWritePos & 65535) >= 8192)
		{
			...
			if (this.m_streamCrossFade == 65536)
			{
				...
				for (int i = 0; i < 8192; i++)
				{
					...
					this.m_streamBuffer[num2++] = (float)num4 * 3.05175781E-05f;
					num2 &= 65535;
				}
			}
			else
			{
				...
				for (int j = 0; j < 8192; j += num7)
				{
					for (int k = 0; k < num7; k++)
					{
						...
						this.m_streamBuffer[num2++] = (float)num10 * 4.656613E-10f;
						num2 &= 65535;
					}
	...
	private void LateUpdate()
	{
		...
		if (!Singleton<LoadingManager>.instance.m_currentlyLoading)
		{
			...
			this.m_listenerInfo.m_dopplerLevel = 0.5f / (float)Mathf.Max(1, Singleton<SimulationManager>.instance.FinalSimulationSpeed);
		}
	...
	{
		this.m_defaultGroup.AddPlayer(0, info, 1f);
	}
	...
	{
		this.m_defaultGroup.AddPlayer(0, clip, 1f, false);
	}
